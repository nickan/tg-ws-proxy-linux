...
from utils.tray_common import (
    ...
    save_config, start_proxy, stop_proxy, tg_proxy_url, is_proxy_running,
)
...
try:
    from ui.gtk_ubuntu_window import GtkUbuntuWindow as AppWindow, send_ipc_show
except Exception as _e:
    from ui.ubuntu_style_window import UbuntuAppWindow as AppWindow, send_ipc_show

_tray_icon: Optional[object] = None
_app_window: Optional[object] = None
_config: dict = {}
_exiting = False
...
def _on_show_window(icon=None, item=None) -> None:
    if _app_window:
        _app_window.restore_window()
...
def _build_menu():
    ...
    return pystray.Menu(
        pystray.MenuItem("Открыть окно приложения", _on_show_window, default=True),
        pystray.Menu.SEPARATOR,
        pystray.MenuItem(t("tray.open_telegram", host=link_host, port=port), _on_open_in_telegram),
...
def run_tray() -> None:
    global _tray_icon, _app_window, _config

    _config = load_config()
    bootstrap(_config)

    # 1. Сначала создаем главное окно приложения
    _app_window = AppWindow(
        config=_config,
        on_start_proxy=lambda: start_proxy(_config, _show_error),
        on_stop_proxy=lambda: stop_proxy(),
        on_restart_proxy=lambda: restart_proxy(_config, _show_error),
        on_exit_app=_on_exit,
        is_proxy_running=is_proxy_running,
        get_proxy_url=lambda: tg_proxy_url(_config),
        on_save_config=lambda cfg: save_config(cfg),
    )

    # 2. Запускаем прокси-службу
    start_proxy(_config, _show_error)

    # 3. Запускаем трей-иконку
    if pystray is not None:
        try:
            _tray_icon = pystray.Icon(APP_NAME, load_icon(), t("app.name"), menu=_build_menu())
            threading.Thread(target=_tray_icon.run, daemon=True, name="tray-thread").start()
            log.info("Tray icon running in background")
        except Exception as e:
            log.warning("Could not start tray icon: %s", e)

    # 4. Запускаем основной цикл событий окна
    _app_window.run()
...
def main() -> None:
    ...
    # Если приложение уже запущено (например в трее), клик по иконке открывает окно!
    if send_ipc_show():
        print("TG WS Proxy уже запущен. Окно открыто.")
        return
    ...
    if not acquire_lock():
        ...
        if send_ipc_show():
            return
        print("TG WS Proxy уже запущен другим процессом.")
        return
